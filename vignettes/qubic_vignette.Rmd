...
There are six functions provide by QUBIC package. 
...
```BCQU()```performs a qualitative biclustering for discretized matrix;
...
```{r quheatmap_function, echo = FALSE}
...
quheatmap = function(x, bicResult, number = 1, showlabel = FALSE, col = c("#313695", 
    ...
    if (length(number) == 1) {
        ...
        number1 = number[[1]]
        bicRows1 = which(res@RowxNumber[, number1])
        bicCols1 = which(res@NumberxCol[number1, ])
        
        number2 = number[[2]]
        bicRows2 = which(res@RowxNumber[, number2])
        bicCols2 = which(res@NumberxCol[number2, ])
        ...
        if (require("fields")) {
            low1 <- floor(min(x1[1:nrow(x1), 1:ncol(x1)]))
            up1 <- ceiling(max(x1[1:nrow(x1), 1:ncol(x1)]))
            low2 <- floor(min(x2[1:nrow(x2), 1:ncol(x2)]))
            up2 <- ceiling(max(x2[1:nrow(x2), 1:ncol(x2)]))
            low <- min(low1, low2)
            up <- max(up1, up2)
            image.plot(zlim = c(low, up), legend.only = TRUE, horizontal = FALSE, 
                col = col, ...)
...
# Draw heatmap of for the 2th bicluster identified in BicatYeast data
...
# Construct the network for the 2thidentified bicluster in BicatYeast
...
stopifnot(require("qgraph"))
qgraph(net[[1]], groups = net[[2]], layout = "spring", minimum = 0.6, legend.cex = 0.5, 
    color = c("red", "blue", "gold", "gray"), edge.label = FALSE)
...
qunet2xml(net, minimum = 0.6, color = cbind(rainbow(length(net[[2]]) - 
    1), "gray"))
...
The E.coli data consists of 4,297 genes and 466 conditions.QUBIC found 83 biclusters.

```{r ecoli, cache = TRUE}
library(QUBIC)

# Load E.coli data
x <- read.table("https://www.sdstate.edu/ps/research/bioinformatics/upload/ecoli_466_4297.txt", 
    header = TRUE)
...
# QUBIC
res <- biclust(M, method = BCQU(), r = 1, q = 0.06, c = 0.95, o = 100, 
    f = 0.25, k = max(ncol(M)%/%20, 2))
system.time(res <- biclust(M, method = BCQU(), r = 1, q = 0.06, c = 0.95, 
    o = 100, f = 0.25, k = max(ncol(M)%/%20, 2)))
summary(res)

```

```{r ecoli heatmap, cache = TRUE, message = FALSE, fig.cap = 'Heatmap for the fifth bicluster identified in the E.coli data. The bicluster consists of 108 genes and 44 conditions', fig.show = 'asis'}
...
par(mar = c(5, 4, 3, 5) + 0.1, mgp = c(0, 1, 0), cex.lab = 1.1, cex.axis = 0.5, 
    cex.main = 1.1)
quheatmap(M, res, number = 5, showlabel = TRUE, col = paleta)

```

```{r ecoli heatmap2, cache = TRUE, message = FALSE, fig.cap = 'Heatmap for the fourth and eighth biclusters identified in the E.coli data.Bicluster #4(topleft) consists of 51 genes and 94 conditions, and bicluster #8 (bottom right) consists of 44 genes and 37 conditions', fig.show = 'asis'}

library(RColorBrewer)
paleta <- colorRampPalette(rev(brewer.pal(11, "RdYlBu")))(11)
par(mar = c(5, 4, 3, 5), cex.lab = 1.1, cex.axis = 0.5, cex.main = 1.1)
quheatmap(M, res, number = c(4, 8), showlabel = TRUE, col = paleta)
...
net <- qunetwork(M, res, number = 5, group = 5, method = "spearman")
stopifnot(require("qgraph"))
qgraph(net[[1]], groups = net[[2]], layout = "spring", minimum = 0.6, 
    color = cbind(rainbow(length(net[[2]]) - 1), "gray"), edge.label = FALSE)
...
# construct the network for the 4th and 8th identified bicluster in
# E.coli data
net <- qunetwork(M, res, number = c(4, 8), group = c(4, 8), method = "spearman")
stopifnot(require("qgraph"))
qgraph(net[[1]], groups = net[[2]], legend.cex = 0.5, layout = "spring", 
