...
The cached yeast expression matrix bundled for this vignette is derived from the
original `BicatYeast` example distributed with the `biclust` package, and is
retained here only to keep this historical QUBIC example reproducible after the
runtime dependency on `biclust` was removed.
