...
```{r cite-qubic, comment=NA, message=FALSE}
citation("QUBIC")
```
...
For comparison and migration purposes, this vignette supports two execution modes.
When `biclust` is available (and not disabled), legacy `biclust` paths are used and
cache files are refreshed. When `biclust` is unavailable (or `options(qubic.disable.biclust = TRUE)`),
the cached baselines are used, and `summary()` for QUBIC results still prints a detailed
cluster report similar to `biclust` output.
...
```{r yeast, cache = TRUE}
library(QUBIC)
yeast_cache_path <- "data/BicatYeast_cached.rds"

if (has_biclust()) {
    data("BicatYeast", package = "biclust")
    saveRDS(BicatYeast, yeast_cache_path, compress = "xz")
} else {
    if (!file.exists(yeast_cache_path)) {
        stop("Missing cached yeast dataset: ", yeast_cache_path,
             ". Install 'biclust' once to refresh cache, or provide this file.")
    }
    BicatYeast <- readRDS(yeast_cache_path)
}
...
res0
res4 <- qubiclust(ecoli, weight = ecoli.weight, verbose = FALSE)
res4
```
...
res5 <- qubiclust(x = ecoli, seedbicluster = res, f = 0.25, verbose = FALSE)
summary(res5)
```
...
```{r biclust-baseline-metadata, cache = TRUE}
baseline_cache_path <- "data/biclust_baseline_results.rds"

if (has_biclust() && methods::isClass("BCQU")) {
    test_for_cache <- ecoli[1:50, , drop = FALSE]
    baseline <- list(
        biclust_version = as.character(utils::packageVersion("biclust")),
        created_at = as.character(Sys.time()),
        dataset = "QUBICdata::ecoli[1:50, ]",
        original_calls = c(
            "res6 <- biclust::biclust(test, method = BCQU(), verbose = FALSE)",
            "res7 <- biclust::biclust(test, method = biclust::BCCC())",
            "res8 <- biclust::biclust(test, method = biclust::BCBimax())"
        ),
        results = list(
            BCQU = biclust::biclust(test_for_cache, method = BCQU(), verbose = FALSE),
            BCCC = biclust::biclust(test_for_cache, method = biclust::BCCC()),
            BCBimax = biclust::biclust(test_for_cache, method = biclust::BCBimax())
        )
    )
    saveRDS(baseline, baseline_cache_path, compress = "xz")
} else {
    if (!file.exists(baseline_cache_path)) {
        stop("Missing cached biclust baseline results: ", baseline_cache_path,
             ". Install 'biclust' once to refresh cache, or provide this file.")
    }
    baseline <- readRDS(baseline_cache_path)
}

cat("Cached biclust version:", baseline$biclust_version, "\n")
cat("Cached at:", baseline$created_at, "\n")
cat("Original calls used for baseline:\n")
for (ln in baseline$original_calls) {
    cat("  ", ln, "\n")
}
```
...
test <- ecoli[1:50,]
if (!exists("baseline")) {
    baseline <- readRDS("data/biclust_baseline_results.rds")
}
res6 <- baseline$results$BCQU
res7 <- baseline$results$BCCC
res8 <- baseline$results$BCBimax
